...
module.exports = {
    ...
    getOpenToken: async param => {
        ...
        let wechatOpenConfig = await models.dbs.config.getWechatOpen();
        if( !wechatOpenConfig
            || !wechatOpenConfig.access_token
            || !wechatOpenConfig.expires_in
            || !(await models.utils.time.checkExpiresInDate({ expiresInDate: wechatOpenConfig.expires_in })) ){
            const newOpenToken = await models.apis.wechatOpen.getOpenToken({
                openAppid: WECHAT_OPEN_APP_ID,
                openAppSecret: WECHAT_OPEN_APP_SECRET,
                ticket: wechatOpenConfig.ticket,
            });
            wechatOpenConfig = await models.dbs.config.updateWechatOpenForce({
                access_token: newOpenToken.component_access_token,
                expires_in: await models.utils.time.createExpiresInDate({ expires_in: newOpenToken.expires_in }),
            });
        }
    ...
    getMpToken: async param => {
        ...
        if( !wechatMpAuthInfo
            ...
            || !(await models.utils.time.checkExpiresInDate({ expiresInDate: wechatMpAuthInfo.expires_in })) ){
            const openToken = await models.wechat.getOpenToken();
            const newMpToken = await models.apis.wechatOpen.refreshAuth({
                openAppid: WECHAT_OPEN_APP_ID,
                openToken: openToken,
                mpAppid: wechatMpAuthInfo.appid,
                mpRefreshToken: wechatMpAuthInfo.refresh_token,
            });
            const ad = await models.dbs.ad.update({
                adId: param.ad._id,
                access_token: newMpToken.authorizer_access_token,
                expires_in: await models.utils.time.createExpiresInDate({ expires_in: newMpToken.expires_in }),
    ...
    updateMpAuthInfo: async param => {
        ...
        const openToken = await models.wechat.getOpenToken();
        const mpAuth = await models.apis.wechatOpen.queryAuth({
            openAppid: WECHAT_OPEN_APP_ID,
            openToken: openToken,
        ...
        if( !haveFuncscope ) {
            await models.dbs.ad.cancelAuth({ appid: mpAuth.authorizer_appid });
            throw new Error('do not have funcscope');
        }

        const mpInfo = await models.apis.wechatOpen.getMpInfo({
            openAppid: WECHAT_OPEN_APP_ID,
            openToken: openToken,
        ...
        const ad = await models.dbs.ad.finishAuth({
            pre_auth_code: param.pre_auth_code,
            appid: mpAuth.authorizer_appid,
            user_name: mpInfo.user_name,
            qrcode_url: qrcode_url,
            access_token: mpAuth.authorizer_access_token,
            expires_in: await models.utils.time.createExpiresInDate({ expires_in: mpAuth.expires_in }),
